...
RSpec.describe 'Superbear CLI' do
  let(:binpath) { File.expand_path('../../../../bin/superbear', __FILE__) }

  it 'prints error message when no file argument' do
    stdout_str, stderr_str, status = Open3.capture3("#{binpath}")
...
HEREDOC
    ...
    it 'shows help with -h' do
      stdout_str, stderr_str, status = Open3.capture3("#{binpath} -h")
    ...
    it 'shows help with --help' do
      stdout_str, stderr_str, status = Open3.capture3("#{binpath} --help")
